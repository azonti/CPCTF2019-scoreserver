<template>
  <div>
    ...
    <div class="toggle row">
      ...
      <div class="col-sm-4"><button :class="['btn', group == 'difficulty' ? 'btn-info' : 'btn-primary']" @click="group='difficulty'">Group by difficulty</button></div>
      ...
      <div v-for="(chals, key) in chalSet">
        ...
        <div class="row">
          <div v-for="challenge in chals" class="col-xl-3 col-md-4 col-sm-6" v-if="!challenge.solved || !hide">
            <router-link :to="{name: 'challenge', params: {challenge_id: challenge.challenge_id}, query: { hide: contestFinished ? 'true' : ''}}" :class="['panel', 'panel-' + challenge.genre, challenge.solved ? 'panel-success' : 'panel-primary']">
...
const seasoner = {
  ...
  difficulty (obj) {
    const nobj = {}
    Object.keys(obj).sort().forEach(key => nobj[`${key*100} (${"★".repeat(key)})`] = obj[key])
    return nobj
  },
  solveCount (obj) {
    const nobj = {}
...
export default {
  ...
  data () {
    return {
      ...
      grouped: {genre: {}, difficulty: {}, solveCount: {}},
      group: "genre"
    }
  },
  created () {
